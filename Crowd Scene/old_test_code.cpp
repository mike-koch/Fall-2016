...
void init() {
	 ...
	 */
	string fileName = "C:\\Sandbox\\School Projects\\Fall-2016\\Crowd Scene\\Body_Mesh_Rigged.obj";
	...
	unsigned int * textureIndexArray = nullptr;
	unsigned int * normalIndexArray = nullptr;

	readOBJFile(fileName, numberOfVerticies, vertices, numberOfNormals, normals, numberOfTextures, textures, numberOfIndicies, indicies, textureIndexArray, normalIndexArray);
	...
	*/
	...
	glBufferData(GL_ARRAY_BUFFER, numberOfVerticies * sizeof(float), vertices, GL_STATIC_DRAW);
	nbrTriangles = numberOfVerticies / 9;
...
void display() {
	...
	vmath::mat4 currentMatrix = vmath::scale(0.1f) * 
 ...
 * sets up the window.
 */
int main2 (int argCount, char *argValues[]) {
