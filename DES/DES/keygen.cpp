...
uint64_t rotate_right(uint64_t original, int number_of_bits_to_rotate);
...
void generate_keys(char *provided_key, uint64_t *keys)
{
	...
	for (int i = 0; i < NUMBER_OF_KEYS_TO_GENERATE; i++) {
		...
		uint64_t right_side_rotated = rotate_right(original_right_side, rotation_amount);
		...
		for (int i = 0; i < 28; i++) {
			int bit_is_set = (right_side_rotated >> i) & 1;

			if (bit_is_set) {
				combined_key |= INT64_C(1) << i;
			}
			else {
				combined_key &= ~(INT64_C(1) << i);
			}
...
void compress_original_key(char *provided_key, uint64_t *compressed_key) {
	...
	for (int i = 0; i < 64; i++) {
		int bit_is_set = (provided_key_as_int >> i) & 1;
		...
		if (BIT_MAP[i]) {
			...
			if (bit_is_set) {
				*compressed_key |= INT64_C(1) << (new_position - 1);
			} else {
				*compressed_key &= ~(INT64_C(1) << (new_position - 1));
			}
		}
...
/* Helper methods to rotate our 28 bit value left or right:
	...
	KNOWN ISSUE: Because I'm rotating a 28-bit value in a 64-bit unsigned int, the MSB (for left rotations) will "bleed" into the 29th bit. This isn't a problem since we won't use the 29th bit anyway.
				 rotate_right has a similar issue, but there are many additional "garbage bits" that are set due to the left shift (since everything is pushed into the unused left bits). 
					They're ignored as well, so not a big issue.
*/
uint64_t rotate_left(uint64_t original, int number_of_bits_to_rotate) {
	return ((original << number_of_bits_to_rotate) | (original >> (28 - number_of_bits_to_rotate)));
}
...
void compress_rotated_key(uint64_t *combined_key, uint64_t *compressed_key) {
	...
	const int BIT_MAP[56] = {
		5, 24, 7, 16, 6, 10, 28, 18, 0, 12, 3, 15, 23, 1,
	...
	for (int i = 0; i < 56; i++) {
		int bit_is_set = (*combined_key >> i) & 1;
